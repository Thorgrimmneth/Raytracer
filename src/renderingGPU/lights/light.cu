...
Light::sampleCylinder(const float3& p_point, RNG* rng) const
{
    ...
    float3 uVec =
        normalize(cross(dir, make_float3(1.f, 0.f, 0.f)));

    if (length(uVec) < 1e-3f)
    {
        uVec =
            normalize(cross(dir, make_float3(0.f, 1.f, 0.f)));
    }
    ...
    float3 normal =
        normalize(randomPos - axisPoint);

    float3 lightDir =
        normalize(randomPos - p_point);

    float dist =
        distance(p_point, randomPos);
...
{
    float dist =
        distance(p_point, getPosition());

    float3 radiance =
        getColorPower() / (dist * dist);
...
Light::sampleQuad(const float3& p_point, RNG* rng) const
{
    ...
    float area =
        length(cross(getDirection(), getV()));
...
) const
{
    const TriangleMesh& mesh =
        scene.triangleMeshes[getGeomIndex()];
    ...
    float sampleArea =
        rng->nextFloat() * mesh.meshArea;

    int triIndex = 0;

    while (
        triIndex < mesh.triangleCount - 1 &&
        mesh.triangleAreaCdf[triIndex] < sampleArea
    )
    {
        ++triIndex;
    }

    const TriangleMeshGeometry& tri =
