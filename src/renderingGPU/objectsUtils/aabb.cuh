...
struct AABB
{
    ...
    D_FORCEINLINE bool intersectCheck(const Ray &ray, float tMin, float tMax, float &outTMin) const
    {
        float nearT = tMin;
        float farT = tMax;

        float3 t1 = (min - ray.origin) * 1.f / ray.direction;
        float3 t2 = (max - ray.origin) * 1.f / ray.direction;
