...
__global__ void shadeWavefrontKernel(CudaScene &scene, float3 *origins, float4 *directions, float3 *throughput,
                                     ...
                                     bool *lastBounceWasDelta, float *lastBsdfPdf, float4 *hitPositions, float4 *hitNormals, 
...
__global__ void shadePlasticKernel(Material *materials, float3 *origins, float4 *directions, float3 *p_throughput,
                                   RNG *p_rng, float4 *p_hitPositions, float4 *p_hitNormals,
                                   int *p_hitMaterialIndices, bool *lastBounceWasDelta, const int *hitQueue, int hitCount, int *nextActiveQueue, int *nextActiveCount, uint depth);
...
__global__ void shadeMirrorKernel(CudaScene scene, float3 *origins, float4 *directions, float3 *p_throughput,
                                  ...
                                  int *p_hitMaterialIndices, const int *hitQueue, int hitCount, int *nextActiveQueue, int *nextActiveCount,
                                  uint depth);
