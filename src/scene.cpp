	...
	void Scene::_spheres()
	{
		...
		for (int i = -11; i < 11; i++)
		{
			for (int j = -11; j < 11; j++)
			{
				...
				if (
					length((center - Vec3f(4.f, 1.f, 0.f))) < minDist ||
					length((center - Vec3f(0.f, 1.f, 0.f))) < minDist ||
					length((center - Vec3f(-4.f, 1.f, 0.f))) < minDist)
					continue;
		...
		_addObject(new Sphere("Sphere2", Vec3f(-4.f, 1.f, 0.f), 1.f));
		_attachMaterialToObject("Emissive", "Sphere2");
		_addObject(new Sphere("Sphere3", Vec3f(4.f, 1.f, 0.f), 1.f));
